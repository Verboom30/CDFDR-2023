...
//draw_table();
        ...
        //PVector Lidar = coord_lidar.get(coord_lidar.size()-1);
        
        LidarX = coord.x+(sin((angle_lidar+angle)*(PI/180))*r)*scale/10;
        LidarY = coord.y-(cos((angle_lidar+angle)*(PI/180))*r)*scale/10;
        ...
        //strokeWeight(5);
        //stroke(255,255,0);
        //ellipse(LidarX*scale/10+coord.x,LidarY*scale/10+coord.y,1*scale,1*scale);
        //ellipse(LidarX,LidarY,1*scale,1*scale);
        
 
         if(LidarX>p2.x && LidarX<p3.x && LidarY>p2.y && LidarY<p4.y){
            strokeWeight(5);
            stroke(0,255,255);
            ellipse(LidarX,LidarY,1*scale,1*scale);
        }
